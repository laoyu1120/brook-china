...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget --no-check-certificate -N "http://download.maobuni.com/brook/brook"
    else
        wget --no-check-certificate -N "http://download.maobuni.com/brook/brook_linux_386"
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate http://download.maobuni.com/brook/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate http://download.maobuni.com/brook/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
...
Install_brook(){
    ...
    Set_iptables
    echo -e "${Info} Brook 安装完成！关注 maobuni.com 最新资讯-默认配置文件为空，请选择 [7.设置 Brook 端口转发 - 1.添加 端口转发] 来添加端口转发。"
}
Start_brook(){
    check_installed_status
    check_pid
